...
int main(int argc, char* argv[])
{
    ...
    mc->setPath(MAIN_CONFIG_PATH);
    MainConfig m = mc->getConfig();

    /* setting up log file */
    Log::getInstance()->setPath(std::string(m.logPath));

    /* Daemonize */
    //daemonize((char *) "/tmp/", const_cast<char*>(m.adaptativeFirewallPidFile.c_str()));
    //syslog(LOG_INFO, "Adaptive firewall running");

    std::string criterionDirectory = std::string(m.criterionDirectory);
    std::string policyDirectory = std::string(m.policyDirectory);
    PolicyApplier* pa = new PolicyApplier(criterionDirectory, policyDirectory);
    pa->loop();
/*
    while(1)
    {
