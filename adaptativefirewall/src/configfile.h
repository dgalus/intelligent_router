...
struct MainConfig
{
  const char* adaptativeFirewallPidFile;
  const char* criterionDirectory;
  const char* logPath;
  const char* policyDirectory;
};
...
class MainConfiguration : public ConfigFile
{
public:
...
private:
  ...
  const std::vector<std::string> validArguments =
        {"log_file", "adaptative_firewall_pid_file", "policy_directory", "criterion_directory"};
};
